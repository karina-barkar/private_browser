...
class AboutPBrowser(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with open(
            "config.json"
        ) as config_file:  # Из файла конфигурации будем брать данные о программе
            config_data = json.load(config_file)
        layout = (
            QVBoxLayout()
        )  # Будем виджеты располагать по вертикали (Vertical layout)
        ...
        layout.addWidget(
            QLabel(
                f"Version: {config_data['application']['version']} "
