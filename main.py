...
class PBMainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        )  # по двойному клику на пустое место будем
        # открывать новую вкладку
        self.tabs.currentChanged.connect(
            self.current_tab_changed
        )  # обработка изменений в текущей вкладке
        ...
        self.add_new_tab(
            QUrl(config_data["application"]["homepage"]), "Homepage"
        )  # откроем Homepage
    ...
    def create_privacy_toolbar(self):
        ...
        self.addToolBar(
            Qt.LeftToolBarArea, privacy_toolbar
        )  # Будет располагаться слева
        proxy_btn = QToolButton(
            privacy_toolbar
        )  # Кнопка с выпадающим меню выбора прокси-сервера
        ...
        for item in range(
            len(self.proxies)
        ):  # Формирование пунктов меню из списка прокси-серверов
            proxy_item = QAction(
                QIcon(flag_path(self.proxies[item][3])),
                f"{self.proxies[item][0]}",
                self,
            )
            proxy_item.triggered.connect(
                lambda _, index=item: self.proxy_clicked(index)
            )  # и добавим слот
        ...
        webrtc_btn = QAction(
            QIcon(res_path(("ic_priv_eye_on.png"))), "Toggle WebRTC", self
        )  # Кнопка WebRTC
        webrtc_btn.setStatusTip("Toggle WebRTC & Javascript")
        webrtc_btn.triggered.connect(
            lambda _, button=webrtc_btn: self.toggle_webrtc(button)
        )  # и слот
    ...
    def toggle_webrtc(self, button):
        ...
        if self.webrtc_enabled:  # Инвертируем состояние вкл WebRTC и Javascript
            ...
            button.setIcon(
                QIcon(res_path(("ic_priv_eye_off.png")))
            )  # Изменяем иконку кнопки на закрытый глаз
        else:
            self.webrtc_enabled = True
            button.setIcon(
                QIcon(res_path(("ic_priv_eye_on.png")))
            )  # Изменяем иконку кнопки на открытый глаз
    ...
    def create_navigation_toolbar(self):
        ...
        )  # начало загрузки страницы по Enter
        nav_toolbar.addWidget(self.urlbar)
        stop_btn = QAction(
            QIcon(res_path("ic_nav_stop.png")), "Stop", self
    ...
    def create_menu_bar(self):
        ...
        menu_bar.setNativeMenuBar(
            False
        )  # Выключение нативного меню ОС - расположение меню будет как на Windows
        file_menu = menu_bar.addMenu("&File")  # Меню File
        new_tab_action = QAction(
            QIcon(res_path("ic_tab_plus.png")), "New Tab", self
        )  # Новая вкладка
        ...
        open_file_action = QAction(
            QIcon(res_path("ic_open_file.png")), "Open file", self
        )  # Открыть html
        open_file_action.setStatusTip("Open from file")
        open_file_action.triggered.connect(self.open_file)
        file_menu.addAction(open_file_action)
        save_file_action = QAction(
            QIcon(res_path("ic_save_file.png")), "Save file", self
        )  # Сохранить html
        ...
        exit_app_action = QAction(
            QIcon(res_path("ic_exit.png")),
            f'Exit {config_data["application"]["name"]}',
            self,
        )
        exit_app_action.setStatusTip(
            f'Exit {config_data["application"]["name"]}'
        )  # Выход из программы
        ...
        navigate_iplocation_action = QAction(
            QIcon(res_path("ic_ext_link.png")), "Visit iplocation.net", self
        )
        ...
        navigate_browserleaks_action = QAction(
            QIcon(res_path("ic_ext_link.png")), "Visit browserleaks.com", self
        )
        ...
        tools_menu.addSeparator()
        navigate_ipleak_action = QAction(
            QIcon(res_path("ic_ext_link.png")), "Visit ipleak.net", self
        )
        ...
        about_action = QAction(
            QIcon(res_path("ic_about.png")),
            f"About {config_data['application']['name']}",
            self,
        )
    ...
    def add_new_tab(self, qurl=None, label="Blank"):
        ...
        )  # связать сигнал подключения и
        # хэндлер обновления адресной строки
        browser.loadFinished.connect(
            lambda _, i=i, browser=browser: self.page_load_finished_handler(  # после окончания загрузки страницы будем
                i, browser
            )
        )  # вызывать этот обработчик
        browser.loadStarted.connect(
            lambda: self.page_load_started_handler(browser)
        )  # свяжем вкл/выкл WebRTC при старте
    ...
    def page_load_finished_handler(self, i, browser):
        ...
        )  # в названии табы отобразим название страницы
        self.tabs.setTabToolTip(
            i, browser.page().title()
        )  # и тултип на всякий случай, если табов будет много
    ...
    def update_title(self, browser):
        ...
        self.setWindowTitle(
            f"{title} - {config_data['application']['name']}"
        )  # обновление названия окна приложения
    ...
    def open_file(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "Open file", "", "HTML (*.htm *.html);;" "All files (*.*)"
        )
        if filename:
            with open(filename, "r") as f:  # откываем файл на чтение
                html_file = f.read()  # читаем файл
            self.tabs.currentWidget().setHtml(
                html_file
            )  # добавляем его содержимое в текущую вкладку
    ...
    def save_file(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save Page", "", "HTML (*.htm *html);;" "All files (*.*)"
        )
    ...
    def update_url_bar(self, url, browser=None):
        ...
        self.urlbar.setStyleSheet(
            f"background-image: url({ssl_icon_path});\n"  # CSS для адресной строки
...
if __name__ == "__main__":  # точка начала выполнения программы
    ...
    private_browser_app = QApplication(
        sys.argv
    )  # само приложение и установка его свойств
    private_browser_app.setApplicationName(config_data["application"]["name"])
    private_browser_app.setApplicationVersion(
        config_data["application"]["version"]
